...
The script infers the repository from `origin` (or from the issue URL), fills missing template sections with `- TODO`, and includes a missing-sections note. Use `--fail-on-missing` in CI/automation when you want strict failure on incomplete issue specs.
